...
from .config import (CHUNK_SIZE, CONFIG_FILENAME, CREDENTIALS_FILE,
                     EMAIL_ADDRS, END_COLUMN, REQUEST_TIMEOUT,
                     SECRETS_DIRECTORY, SOURCE_SPREADSHEET, START_COLUMN,
                     TRACKED_SHEET_HEADER_OFFSET, TRACKED_SHEET_NAME)
...
class GSheets:
    def __init__(self):
        httpAuth = credentials.authorize(
            httplib2.Http(timeout=REQUEST_TIMEOUT))  # Авторизуемся в системе
        ...
        if not self.spreadsheet_id or not self.main_sheet_id or not self.start_page_token:
            print(
                f'GSheets: A config {CONFIG_FILENAME} doesn\'t match following format requirements')
            pprint({
    ...
    def _dump_config(self):
        with open(os.path.join(SECRETS_DIRECTORY, CONFIG_FILENAME),
                  'w') as config_file:
            json.dump({
    ...
    def batch(self, requests: list):
        try:
            ...
            print(
                f'GSheets error: unable to fetch {self.spreadsheet_id} table with Google Sheets API:',
                e)
            return {}
    ...
    def get_chunk(self, start_idx):
        ...
        try:
            ...
            return (
            start_idx + CHUNK_SIZE, *range_from_to, response.get("values"))
